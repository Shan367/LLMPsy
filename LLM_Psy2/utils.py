...
def hypothesis_testing(result1, result2, significance_level, model, crowd_name):
    ...
    df = n1 + n2 - 2 if equal_var else ((std1 ** 2 / n1 + std2 ** 2 / n2) ** 2) / (
...
def analysis_results(questionnaire, args):
    ...
    output_list += f'| Category | {model} (n = {len(test_data)}) | ' + ' | '.join(
        [f'{c[0]} (n = {c[1]})' for c in crowd_list]) + ' |\n'
    ...
    for cat_index, cat in enumerate(questionnaire['categories']):
        ...
        for crowd_index, crowd_group in enumerate(crowd_list):
            crowd_data = (
            cat["crowd"][crowd_index]["mean"], cat["crowd"][crowd_index]["std"], cat["crowd"][crowd_index]["n"])
            result_text, result_list = hypothesis_testing(test_results[cat_index], crowd_data, significance_level,
                                                          model, crowd_group[0])
        ...
        output_list += '\n'
    plot_bar_chart(mean_list, cat_list, [model] + [c[0] for c in crowd_list], save_name=args.figures_file,
...
def run(args,generator):
    questionaireList=['BFI', 'DTDD', 'EPQ-R', 'ECR-R', 'CABIN', 'GSE', 'LMS', 'BSRI', 'ICB', 'LOT-R', 'Empathy', 'EIS', 'WLEIS', '16P']\
